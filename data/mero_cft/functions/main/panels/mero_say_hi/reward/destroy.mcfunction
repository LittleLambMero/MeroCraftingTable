# == 拆除經驗產生器 == #
execute unless entity @s[nbt = {SelectedItem: {id: "minecraft:written_book", tag: {ItemID: "mero_cft:destroy_reward"}}}] run function mero_cft:main/execute_failed
execute unless entity @s[nbt = {SelectedItem: {id: "minecraft:written_book", tag: {ItemID: "mero_cft:destroy_reward"}}}] run return -1
