execute unless entity @s[nbt = {SelectedItem: {id: "minecraft:written_book", tag: {ItemID: "mero_cft:destroy_xp_spawner"}}}] run function mero_cft:main/execute_failed
execute unless entity @s[nbt = {SelectedItem: {id: "minecraft:written_book", tag: {ItemID: "mero_cft:destroy_xp_spawner"}}}] run return -1
